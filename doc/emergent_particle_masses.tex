...
\fbox{%
\begin{minipage}{\dimexpr\linewidth-2\fboxsep-2\fboxrule\relax}
\textbf{Key Result:} Quark masses follow $m_{\text{eff},n} = m_0 [(2n+1)^p (1 + \epsilon n(n-1) - \delta)]^3 (1 - \eta_n)$, with $p_{\text{up}} \approx 0.618 + 0.01 n$, $p_{\text{down}} \approx 1.618 - 0.01 n$, $\epsilon \approx 0.55$, $\delta \approx 0.002 n^2$, and leakage $\eta_n \approx \Lambda_{\text{QCD}} / m_n$. Tension limits fractional stability, predicting hierarchy with improved fits via curvature.

\textbf{Verification:} SymPy confirms scaling and overlap calculations (e.g., solve for $R$ in extended $E(R)$); code at \url{https://github.com/trevnorris/vortex-field}.
\end{minipage}
}
}
