...
With $v_{\text{eff}}^2 = g \rho_{4D}^0 / m$.

Step 4: Helmholtz Decomposition and Twist Source

Decompose $\delta\mathbf{v} = - \nabla_4 \Phi + \nabla_4 \times \mathbf{B}_4$ (P-3).

Scalar part (gravity-like, compressible): As above for gravity.

Vector part (EM-like, incompressible): From curl Euler, $\partial_t (\nabla_4 \times \delta\mathbf{v}) = 0$ (base linear), but twist adds source from background cross terms.
...
From curl Euler, $\partial_t \delta\boldsymbol{\omega} = - \nabla_4 \times [(\mathbf{v}_{\text{background}} \cdot \nabla_4) \delta\mathbf{v} + (\delta\mathbf{v} \cdot \nabla_4) \mathbf{v}_{\text{background}}]$ (nonlinear cross, but for effective source, approximates to source ~ curl ($\mathbf{v}_{\text{twist}} \times \delta\mathbf{v}$) ~ $\tau c \delta^2 \times \delta\mathbf{v}$ component)

In effective, the vector equation is $\partial_t \delta\boldsymbol{\omega} = \nabla_4 \times (\text{source}_{\text{twist}})$, where source_{\text{twist}} = (\tau c / \rho_0) \mathbf{n} \delta^2$

Then $\partial_t (\nabla_4 \times (\nabla_4 \times \mathbf{B}_4)) = \nabla_4 \times (\text{source})$

But for wave, to include propagation, the full is from the incompressible limit, where the pressure enforces the wave.
...
Substituting into the linearized equations with twist sources as inhomogeneities yields Maxwell equations (Gaussian convention, $\epsilon_0 = 1$):
