\subsection{2.0 Introduction to Mathematical Framework}
...
\subsubsection{The Golden Ratio in Vortex Stability}

The golden ratio $\phi = (1+\sqrt{5})/2 \approx 1.618$ emerges naturally from energy minimization in hierarchical vortex structures. This is not a fitted parameter but a mathematical consequence of avoiding resonant reconnections.

Consider vortices with nested radii $R_0$, $R_1$, $R_2$, \dots To minimize reconnection risk, adjacent vortices must have incommensurate phase relationships. The energy for braiding includes:

\textbf{Bending attraction}: $E_{\text{bend}} \propto -1/R$ (phase gradients favor smaller $R$)

\textbf{Interaction repulsion}: $E_{\text{int}} \propto 1/(R_{n+1} - R_n)^2$ (separation prevents reconnection)

For successive radii with ratio $x = R_{n+1}/R_n$, minimizing total energy:
\begin{equation}
E(R_{n+1}) = -1/R_{n+1} + 1/(R_{n+1} - R_n)^2
\end{equation}

Setting $dE/dR_{n+1} = 0$ and solving yields $x^2 - x - 1 = 0$, giving $x = \phi$.

This golden ratio scaling appears throughout our framework:
\begin{itemize}
\item Lepton mass hierarchies scale as $(2n+1)^\phi$
\item Fine structure constant involves $\phi^{-2}$
\item Neutrino helical twist $\theta_{\text{twist}} = \pi/\sqrt{\phi}$
\end{itemize}
...
This sector corresponds to pure suck: irrotational flow ($\nabla \times \mathbf{v} = 0$) creating attractive pressure gradients, analogous to a bathtub drain pulling water inward. From the Helmholtz decomposition, the scalar potential $\Phi$ satisfies the wave equation derived from combining the linearized continuity and Euler:
...
Consider a vortex draining aether from 3D space into the 4D bulk. At the core boundary (radius $\xi$), we have:

\paragraph{Boundary Regions:}
\begin{itemize}
\item 3D region: $r > \xi$, $w = 0$
\item Transition: $r \approx \xi$, $|w| < \xi$  
\item 4D drainage: $r < \xi$, $w \in (-\infty,\infty)$
\end{itemize}

As fluid crosses this boundary, angular momentum must be conserved but its form changes:
\begin{equation}
L_{3D} = r \times p  \quad (\text{only } x,y,z \text{ components})
\end{equation}
\begin{equation}
L_{4D} = \text{generalized } r \times p  \quad (\text{includes } w\text{-component})
\end{equation}

The mismatch creates a field correction:
\begin{equation}
L_{3D} = \text{Projection}(L_{4D}) + E\times B/c^2
\end{equation}

This field correction IS the electromagnetic field!

\paragraph{Derivation of Charge}
...
where $\Box = \partial_t^2 / c^2 - \nabla^2$. The tsunami principle (Section 2.5) distinguishes bulk longitudinal adjustments ($v_L > c$, unobservable) from observable transverse shake at $c$. To derive these, consider the linearized GP for transverse perturbations on vortices: The Kelvin wave dispersion $\omega^2 = c^2 k^2 + \omega_0^2$ (with cutoff $\omega_0 \sim v_L / \xi$), projecting to the d'Alembertian form for far-field radiation. For GW, quadrupole sources arise from vortex motion asymmetries; for photons, current from helical oscillations. SymPy confirms the wave solutions and dispersion relations; code at \url{https://github.com/trevnorris/vortex-field}.
...
\begin{itemize}
\item \textbf{Direct Intersection}: The vortex sheet crosses $w=0$ at a point, contributing base circulation $\Gamma$.
...
\item \textbf{Induced $w$-Flow}: Drainage along $w$ creates a corrective flow, contributing the final $\Gamma$.
\end{itemize}
