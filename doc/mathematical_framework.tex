...
\hline
\multicolumn{4}{|l|}{\emph{Loop energetics and rim-mode symbols used later}} \\
\hline
...
Apply Helmholtz decomposition (P-4) to $\delta \mathbf{v}_4 = -\nabla_4 \chi + \nabla_4 \times \mathbf{B}_4$, separating compressible (scalar $\chi$ $[L^2 T^{-1}]$) and incompressible (vector $\mathbf{B}_4$ $[L^2 T^{-1}]$) parts, now with oscillatory modulation in the phase. Taking $\nabla_4 \cdot$ on Euler gives:

\begin{equation}
\partial_t (\nabla_4 \cdot \delta \mathbf{v}_4) = -v_{\text{eff}}^2 \nabla_4^2 (\delta \rho_{4D} / \rho_{4D}^0) - \nabla_4^2 \delta Q,
\end{equation}

and substituting $\nabla_4 \cdot \delta \mathbf{v}_4 = -\nabla_4^2 \,\chi$ yields the scalar precursor. From linearized continuity:
...
\begin{equation}
\partial_{tt} \,\chi - v_{\text{eff}}^2 \,\nabla_4^2 \,\chi = v_{\text{eff}}^2 \sum_i \frac{\dot{M}_i}{\rho_{4D}^0} \,\delta^4(\mathbf{r}_4 - \mathbf{r}_{4,i}) + v_{\text{eff}}^2 \,\nabla_4^2 \delta Q / \rho_{4D}^0.
\end{equation}
