...
% ---- Foundational Postulates (Card Layout) ----
% Caption + label so existing \ref{tab:postulates} continues to work
\begin{center}
...
\postulate{P-1}{
Compressible 4D medium with GP dynamics.
}{
Continuity: $\partial_t \rho_{4D} + \nabla_4 \cdot (\rho_{4D}\mathbf{v}_4)=0$;\;
Euler: $\partial_t \mathbf{v}_4 + (\mathbf{v}_4 \cdot \nabla_4)\mathbf{v}_4 = -(1/\rho_{4D})\,\nabla_4 P$;\;
Barotropic EOS: $P = (g/2m^{2})\,\rho_{4D}^{2}$.
}{
Intake/Twist-dominant.
}{
Compressible superfluid; defects are holes where phase winds.
}
\medskip

\postulate{P-2}{
Vortex sinks drain into the extra dimension.
}{
Sink term: $-\sum_i \dot{M}_i \,\delta^4(\mathbf{r}_4 - \mathbf{r}_{4,i})$;\;
Sink strength: $\dot{M}_i = \rho_{4D}^0\,\Gamma_i\,\xi_c^2$.
}{
Intake.
}{
Cores act as tiny drains into $\pm w$; no net depletion on the slice.
}
\medskip

\postulate{P-3}{
Dual wave modes (bulk $v_L$, vortex oscillations $c$).
}{
Longitudinal: $v_L = \sqrt{g \,\rho_{4D}^0 / m^{2}}$;\;
Transverse: $c$ emergent from vortex dynamics;\;
Effective: $v_{\text{eff}} = \sqrt{g \,\rho_{4D}^{\text{local}} / m^{2}}$.
}{
Waves (with Intake/Twist in the bulk).
}{
Bulk re-equilibrates quietly; surface waves manifest as photons.
}
\medskip

\postulate{P-4}{
Helmholtz decomposition on the slice (Slope potential + Eddies solenoidal), with Intake across the slab.
}{
$\mathbf{v}_4 \;=\; -\nabla_4 \varphi \;+\; \nabla_4 \times \mathbf{B}_4$.
}{
Slope + Eddies/Drag.
}{
Local donut slice around sheet gives textbook $1/\rho$ swirl.
}
\medskip

\postulate{P-5}{
Projection invariance of circulation.
}{
Circulation: $\Gamma = n\,\kappa$, $\;\kappa = 2\pi \hbar / m$;\;
a slice loop linking once measures $\Gamma$;\;
half-spaces contribute $\Gamma/2$ each;\;
potential fields contribute $0$.
Vortices as tori/sheets with \textbf{Twist}
(quantized cross-slab circulation; $\theta+\tau w$).
}{
Eddies/Drag (with Waves).
}{
Any loop linking once reads the same $\Gamma$, regardless of slab thickness.
}
\medskip

\postulate{P-6}{
Discrete vortex projection.
}{
Projection uses $\sum_i$ not $\int dw$;\;
vortices intersect at points $\{(\mathbf r_i, w_i)\}$;\;
observable quantities aggregate discretely.
}{
All modes (projection).
}{
Sheets pierce the slice at points—count them, don’t average.
}
...
\paragraph{No observable ``aether wind'' at leading order.}
...
\subsubsection{Local Energy Balance}
...
\paragraph{Projected fields and units.}
...
Analogously, projecting the $4$D momentum balance $\partial_t(\rho_{4D}\mathbf{v}_{\parallel})+\nabla\!\cdot T^{\parallel}_{4D}+\partial_w T^{w\parallel}_{4D}=0$ gives
\begin{equation}
...
\paragraph{Quantum pressure: specific vs.\ per-volume force.}
...
\label{eq:E-GP}
\end{equation}
\paragraph*{Closed-core energetics (constants used later).}
...
\paragraph*{Notation handoff: rim phase and internal modes.}
