...
Based on doc/projected_em.tex, section on homogeneous Maxwell equations.
"""
...
def test_dimensional_consistency(v):
    """
    Test dimensional consistency of homogeneous Maxwell equations.
    ...
    v.success("Dimensional consistency verified")


def test_vector_calculus_identities(v):
    """
    Test fundamental vector calculus identities underlying the laws.
    ...
    v.success("Vector calculus identities verified")


def test_faraday_law_derivation(v):
    """
    Test the mathematical derivation of Faraday's law from potentials.
    ...
    dt_curl_A_dims = v.dt(v.curl_dim(v.get_dim('A')))      # ∂_t(∇×A) = ∂_t B
    ...
    v.success("Faraday's law derivation verified")


def test_physical_interpretation(v):
    """
    Test physical interpretation and units consistency.
...
def test_projection_identities(v):
    """
    Test projection-specific identities and cancellation mechanisms.
    ...
    dt_curl_A_dims = v.dt(v.curl_dim(v.get_dim('A')))
    curl_dt_A_dims = v.curl_dim(v.dt(v.get_dim('A')))
    ...
    v.success("Projection identities verified")


def test_two_em_laws():
    """
    Main test function for Two EM Laws that are Pure Kinematics.

    This function coordinates all verification tests for the homogeneous
    Maxwell equations that emerge as pure kinematic identities.
    ...
    test_dimensional_consistency(v)

    v.info("\n--- 2) Vector Calculus Identities ---")
    test_vector_calculus_identities(v)

    v.info("\n--- 3) Faraday's Law Derivation ---")
    test_faraday_law_derivation(v)

    v.info("\n--- 4) Physical Interpretation ---")
    test_physical_interpretation(v)

    v.info("\n--- 5) Projection Identities ---")
    test_projection_identities(v)
