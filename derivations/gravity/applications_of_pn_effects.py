...
def test_binary_pulsar_timing_and_decay(v):
    ...
    # Verify the periastron advance equation from the paper: ω̇ = 3(2π/P_b)^(5/3)(GM/c³)^(2/3)/(1-e²)
    expected_periastron_advance = 3 * (2*pi/P_b)**(Rational(5,3)) * (G*M_total/c**3)**(Rational(2,3)) / (1 - e**2)
    v.check_eq("Periastron advance formula", periastron_advance, expected_periastron_advance)
    ...
    # Verify energy loss equation from paper: Ė = -(32/5) G μ² a⁴ Ω⁶ / c⁵
    # Note: quadrupole_energy_loss already has the negative sign
    expected_energy_loss = quadrupole_energy_loss  # This is the correct formula
    v.check_eq("Quadrupole energy loss formula", quadrupole_energy_loss, -Rational(32, 5) * G * mu**2 * a**4 * Omega**6 / c**5)
    ...
    # Verify the period decay equation from paper: Ṗ_b/P_b = -(192π/5)(GM/c³)(2π/P_b)^(5/3) f(e)
    expected_period_decay = -(192*pi/5) * (G*M_total/c**3) * (2*pi/P_b)**(Rational(5,3)) * f_e
    v.check_eq("Binary period decay formula", period_decay_rate, expected_period_decay)

    # Verify the eccentricity function f(e) = (1-e²)^(-7/2) (1 + 73e²/24 + 37e⁴/96)
    expected_f_e = (1 - e**2)**(-Rational(7,2)) * (1 + 73*e**2/24 + 37*e**4/96)
    v.check_eq("Eccentricity function f(e)", f_e, expected_f_e)
...
def test_gravitational_waves_from_mergers(v):
    ...
    # Verify the gravitational wave strain equation: h_+ = (4Gμ/c²r)(GMΩ/c³)^(2/3) cos(2φ)
    cos_2phi = sp.cos(2*phi)
    expected_strain = strain_factor * pn_amplitude * cos_2phi
    gw_strain = strain_factor * pn_amplitude * cos_2phi
    v.check_eq("GW strain amplitude formula", gw_strain, expected_strain)
    ...
    # Verify chirp mass extraction formula: M_chirp = (c³/G)(df/dt / f^(11/3))^(3/5) / (96π^(8/3)/5)^(3/5)
    expected_chirp_mass = (c**3/G) * (df_dt / f**(Rational(11,3)))**(Rational(3,5)) / (96 * pi**(Rational(8,3)) / 5)**(Rational(3,5))
    v.check_eq("Chirp mass extraction formula", chirp_mass, expected_chirp_mass)
    ...
    # Verify quasi-normal mode frequency: ω ≈ 0.5 c³/(GM)
    expected_qnm = Rational(1,2) * c**3 / (G * M_total)
    v.check_eq("Quasi-normal mode frequency", qnm_frequency, expected_qnm)
    ...
    # Verify general waveform scaling from paper: h ~ (GM/c²r)(v/c)²
    expected_general_scaling = (G * M_total / (c**2 * r)) * (velocity_ratio)**2
    v.check_eq("General GW scaling formula", general_waveform_scale, expected_general_scaling)
...
def test_frame_dragging_earth_orbit_gyroscopes(v):
    ...
    # Verify vector potential equation: A_g = G(J × r)/r³
    expected_vector_potential = G * J_angular / r**3  # Taking magnitude for scalar verification
    v.check_eq("Gravitomagnetic vector potential", vector_potential_magnitude, expected_vector_potential)
    ...
    # Verify Lense-Thirring precession formula from paper: Ω = 3GJ/(2c²r³)
    expected_lt_precession = 3 * G * J_angular / (2 * c**2 * r**3)
    v.check_eq("Lense-Thirring precession formula", lt_precession, expected_lt_precession)
    ...
    # Verify Earth angular momentum formula: J = (2/5)MR²ω
    expected_j_earth = Rational(2,5) * M_earth * R_earth**2 * omega_earth
    v.check_eq("Earth angular momentum formula", j_earth_scale, expected_j_earth)

    # Verify Earth LT precession scaling
    expected_earth_lt = G * j_earth_scale / (c**2 * R_earth**3)
    v.check_eq("Earth LT precession scaling", earth_lt_scale, expected_earth_lt)
