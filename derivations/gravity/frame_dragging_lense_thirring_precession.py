...
def test_lense_thirring_general_formula(v):
    ...
    # Add mathematical equation verification
    v.info("Testing mathematical structure of Lense-Thirring formula")

    # Test the general vector formula structure: Ω_LT = (G/c²r³)[3(J·r̂)r̂ - J]
    # For x-component: Ω_x = (G/c²r³)[3(J·r̂)(r_x/r) - J_x]
    expected_Omega_x = (G / (c**2 * r**3)) * (3 * J_dot_r_hat * (rx/r) - Jx)
    v.check_eq("Lense-Thirring x-component formula", Omega_LT_x, expected_Omega_x)

    # For y-component: Ω_y = (G/c²r³)[3(J·r̂)(r_y/r) - J_y]
    expected_Omega_y = (G / (c**2 * r**3)) * (3 * J_dot_r_hat * (ry/r) - Jy)
    v.check_eq("Lense-Thirring y-component formula", Omega_LT_y, expected_Omega_y)

    # For z-component: Ω_z = (G/c²r³)[3(J·r̂)(r_z/r) - J_z]
    expected_Omega_z = (G / (c**2 * r**3)) * (3 * J_dot_r_hat * (rz/r) - Jz)
    v.check_eq("Lense-Thirring z-component formula", Omega_LT_z, expected_Omega_z)
...
def test_equatorial_and_polar_limits(v):
    ...
    # Add mathematical equation verification for special cases
    v.info("Testing mathematical equations for equatorial and polar cases")

    # Equatorial case: J·r̂ = 0, so Ω_LT = (G/c²r³)[3(0)r̂ - J] = -GJ/(c²r³)
    J_dot_r_hat_eq = 0  # Equatorial: J perpendicular to r
    Omega_equatorial = (G / (c**2 * r**3)) * (3 * J_dot_r_hat_eq - J)
    expected_equatorial = -G * J / (c**2 * r**3)
    v.check_eq("Equatorial plane formula", Omega_equatorial, expected_equatorial)

    # Polar case: J·r̂ = J, so Ω_LT = (G/c²r³)[3J - J] = 2GJ/(c²r³)
    J_dot_r_hat_polar = J  # Polar: J parallel to r
    Omega_polar = (G / (c**2 * r**3)) * (3 * J_dot_r_hat_polar - J)
    expected_polar = 2 * G * J / (c**2 * r**3)
    v.check_eq("Polar axis formula", Omega_polar, expected_polar)

    # Test the factor relationship: polar = -2 × equatorial
    v.check_eq("Polar to equatorial ratio", Omega_polar / (-expected_equatorial), 2)
...
def test_gem_vector_potential(v):
    ...
    # Add mathematical equation verification for GEM vector potential
    v.info("Testing mathematical structure of GEM vector potential formula")

    # Test the vector potential formula: A_g = (2G/c²)(J×r)/r³
    # For x-component: A_g_x = (2G/c²)(J_y * r_z - J_z * r_y)/r³
    expected_A_g_x = (2 * G / c**2) * (Jy * rz - Jz * ry) / r**3
    v.check_eq("GEM vector potential x-component", A_g_x, expected_A_g_x)

    # For y-component: A_g_y = (2G/c²)(J_z * r_x - J_x * r_z)/r³
    expected_A_g_y = (2 * G / c**2) * (Jz * rx - Jx * rz) / r**3
    v.check_eq("GEM vector potential y-component", A_g_y, expected_A_g_y)

    # For z-component: A_g_z = (2G/c²)(J_x * r_y - J_y * r_x)/r³
    expected_A_g_z = (2 * G / c**2) * (Jx * ry - Jy * rx) / r**3
    v.check_eq("GEM vector potential z-component", A_g_z, expected_A_g_z)

    # Test cross product identity verification
    v.check_eq("Cross product x-component identity", J_cross_r_x, Jy * rz - Jz * ry)
    v.check_eq("Cross product y-component identity", J_cross_r_y, Jz * rx - Jx * rz)
    v.check_eq("Cross product z-component identity", J_cross_r_z, Jx * ry - Jy * rx)
