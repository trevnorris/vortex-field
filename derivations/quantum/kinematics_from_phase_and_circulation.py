...
def test_physical_consistency_checks(v):
    ...
    hbar_eff = symbols('hbar_eff', real=True, positive=True)
    m, p_val = symbols('m p_val', real=True, positive=True)

    v.info("Testing overall physical consistency")

    # Test 1: Dispersion relation consistency from doc/quantum.tex eq. (174)
    # ω(k) = ℏk²/(2m) for free particle
    k_val = p_val / hbar_eff  # de Broglie relation k = p/ℏ
    omega_from_k = hbar_eff * k_val**2 / (2 * m)
    ...
    # Test 2: Energy-frequency relation E = ℏω consistency
    # Using E = p²/(2m) and ω = E/ℏ should give the same ω as dispersion relation
    kinetic_energy = p_val**2 / (2 * m)
    omega_from_energy = kinetic_energy / hbar_eff

    v.check_eq("Energy-frequency consistency: ω = E/ℏ", omega_from_energy, expected_omega)

    # Test 3: de Broglie wavelength consistency
    ...
    # Test 4: Phase and group velocity relationships
    # v_phase = ω/k, v_group = dω/dk
    phase_vel = omega_from_k / k_val
    ...
    # Test 5: Verify group velocity is twice phase velocity (standard quantum result)
    v.check_eq("Group/phase velocity relation: v_g = 2v_p", expected_group_vel, 2 * expected_phase_vel)

    # Test 6: Dimensional consistency checks
    # Use existing physics dimensions from the helper
    hbar_dim = v.M * v.L**2 / v.T  # Action dimension
    momentum_dim = v.M * v.L / v.T  # Momentum dimension
    mass_dim = v.M  # Mass dimension

    # Check that kinetic energy formula gives correct energy dimensions
    # p²/(2m) should have dimensions [M L² T⁻²]
    energy_formula_dim = momentum_dim**2 / mass_dim
    v.check_dims("Kinetic energy p²/(2m) dimensions", energy_formula_dim, v.M * v.L**2 / v.T**2)

    # Check that de Broglie wavelength formula gives length dimensions
    # 2πℏ/p should have dimensions [L]
    wavelength_formula_dim = hbar_dim / momentum_dim
    v.check_dims("de Broglie λ = 2πℏ/p dimensions", wavelength_formula_dim, v.L)

    # Check that frequency formula gives inverse time dimensions
    # E/ℏ should have dimensions [T⁻¹]
    frequency_formula_dim = (momentum_dim**2 / mass_dim) / hbar_dim
    v.check_dims("Angular frequency ω = E/ℏ dimensions", frequency_formula_dim, 1/v.T)

    # Check that wave vector formula gives inverse length dimensions
    # p/ℏ should have dimensions [L⁻¹]
    wavenumber_formula_dim = momentum_dim / hbar_dim
    v.check_dims("Wave vector k = p/ℏ dimensions", wavenumber_formula_dim, 1/v.L)

    # Test 7: Verify circulation quantization is preserved in kinematics
    # From eq. (32): ∮ ∇S · dl = 2πnℏ_eff has dimensions of action
    # This should match momentum × length scale
    circulation_dim = hbar_dim  # ℏ has action dimensions
    momentum_length_dim = momentum_dim * v.L
    v.check_dims("Circulation-momentum scale consistency", circulation_dim, momentum_length_dim)
